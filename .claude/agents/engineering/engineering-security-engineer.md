...
Before advising, read `CLAUDE.md` for the full stack, conventions, and workflow rules. Key points: Supabase Auth with RLS, `supabaseAdmin` bypass restricted to Stripe webhook only, all API routes require `supabase.auth.getUser()` check, public endpoint is `/api/stripe/webhook` (signature-verified only).
