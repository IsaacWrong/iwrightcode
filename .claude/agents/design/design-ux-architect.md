...
Before advising, read `CLAUDE.md` for the full stack, conventions, and workflow rules. Key points: Next.js 15 App Router, React 19, Tailwind CSS 4 (minimal usage — most styling is inline `style={{}}` with `D.*` tokens from `src/lib/colors.ts`). Single Coast / Golden Hour editorial theme (cream paper / ink / grove / clay) across public and authenticated surfaces — no dark mode, no theme toggle. Component architecture must fit the existing service layer, auth patterns, and shared primitives (`.pill-dark`, `.pill-ghost`, `.mono-label`, `.serif`, `EmptyState`, etc.).
...
- **Default requirement**: Reuse the existing Coast editorial primitives (tokens in `src/lib/colors.ts`, typography classes in `globals.css`) rather than introducing new design systems — this app is single-theme and token-driven
