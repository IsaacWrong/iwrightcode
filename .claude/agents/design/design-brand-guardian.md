...
Before advising, read `CLAUDE.md` for the full stack, conventions, and workflow rules. Key points: the app is a single-theme Coast / Golden Hour editorial system — cream paper (`#f6f2e9`), ink (`#0b110c`), grove green (`#2c6b45`), clay orange (`#c97a3b`). Fraunces display, Source Serif body, Inter UI, JetBrains Mono for mono-labels. No dark mode, no theme toggle, no gradients, no drop shadows. Tokens live in `src/lib/colors.ts` (`D.*`) and CSS vars in `globals.css`. All brand recommendations must align with this existing visual system, not generic brand strategy.
